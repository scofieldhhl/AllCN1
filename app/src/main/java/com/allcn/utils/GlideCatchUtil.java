...
//        try {
//            return getFormatSize(getFolderSize(new File(AppMain.ctx().getCacheDir() + "/" + GlideCatchConfig.GLIDE_CARCH_DIR)));
//        } catch (Exception e) {
//            e.printStackTrace();
//            return "获取失败";
//        }
...
//        return deleteFolderFile(AppMain.ctx().getCacheDir() + "/" + GlideCatchConfig.GLIDE_CARCH_DIR, true);
//    }
        ...
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.get(AppMain.ctx()).clearDiskCache();
                    }
                }).start();
            } else {
                Glide.get(AppMain.ctx()).clearDiskCache();
    ...
    public boolean clearCacheMemory() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //只能在主线程执行
                Glide.get(AppMain.ctx()).clearMemory();
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
